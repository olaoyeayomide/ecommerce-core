...
if TYPE_CHECKING:
    from paystack.tax._calculation_line_item import CalculationLineItem
...
class Calculation(CreateableAPIResource["Calculation"]):
    ...
    Related guide: [Calculate tax in your custom payment flow](https://paystack.com/docs/tax/custom)
    """
    ...
    class ShipFromDetails(paystackObject):
        class Address(paystackObject):
            city: Optional[str]
            """
            City, district, suburb, town, or village.
    ...
    class ShippingCost(paystackObject):
        class TaxBreakdown(paystackObject):
            class Jurisdiction(paystackObject):
                country: str
                """
                Two-letter country code ([ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2)).
                """
            ...
            class TaxRateDetails(paystackObject):
                display_name: str
                """
            ...
            amount: int
            """
            The amount of tax, in the [smallest currency unit](https://paystack.com/docs/currencies#zero-decimal).
            """
            ...
            taxable_amount: int
            """
            The amount on which tax is calculated, in the [smallest currency unit](https://paystack.com/docs/currencies#zero-decimal).
            """
            _inner_class_types = {
                "jurisdiction": Jurisdiction,
        ...
        amount: int
        """
        The shipping amount in the [smallest currency unit](https://paystack.com/docs/currencies#zero-decimal). If `tax_behavior=inclusive`, then this amount includes taxes. Otherwise, taxes were calculated on top of this amount.
        """
        amount_tax: int
        """
        The amount of tax calculated for shipping, in the [smallest currency unit](https://paystack.com/docs/currencies#zero-decimal).
        """
        shipping_rate: Optional[str]
        """
        The ID of an existing [ShippingRate](https://paystack.com/docs/api/shipping_rates/object).
        """
        ...
        tax_code: str
        """
        The [tax code](https://paystack.com/docs/tax/tax-categories) ID used for shipping.
        """
        _inner_class_types = {"tax_breakdown": TaxBreakdown}

    class TaxBreakdown(paystackObject):
        class TaxRateDetails(paystackObject):
            class FlatAmount(paystackObject):
                amount: int
                """
    ...
    class CreateParamsCustomerDetails(TypedDict):
        ...
        """
        The customer's tax IDs. paystack Tax might consider a transaction with applicable tax IDs to be B2B, which might affect the tax calculation result. paystack Tax doesn't validate tax IDs for correctness.
        """
    ...
    class CreateParamsLineItem(TypedDict):
        amount: int
        """
        A positive integer representing the line item's total price in the [smallest currency unit](https://paystack.com/docs/currencies#zero-decimal).
        ...
        """
        A custom identifier for this line item, which must be unique across the line items in the calculation. The reference helps identify each line item in exported [tax reports](https://paystack.com/docs/tax/reports).
        """
        ...
        """
        A [tax code](https://paystack.com/docs/tax/tax-categories) ID to use for this line item. If not provided, we will use the tax code from the provided `product` param. If neither `tax_code` nor `product` is provided, we will use the default tax code from your Tax Settings.
        """
    ...
    class CreateParamsShippingCost(TypedDict):
        ...
        """
        A positive integer in the [smallest currency unit](https://paystack.com/docs/currencies#zero-decimal) representing the shipping charge. If `tax_behavior=inclusive`, then this amount includes taxes. Otherwise, taxes are calculated on top of this amount.
        """
        shipping_rate: NotRequired[str]
        """
        If provided, the [shipping rate](https://paystack.com/docs/api/shipping_rates/object)'s `amount`, `tax_code` and `tax_behavior` are used. If you provide a shipping rate, then you cannot pass the `amount`, `tax_code`, or `tax_behavior` parameters.
        """
        tax_behavior: NotRequired[Literal["exclusive", "inclusive"]]
        """
        ...
        tax_code: NotRequired[str]
        """
        The [tax code](https://paystack.com/docs/tax/tax-categories) used to calculate tax on shipping. If not provided, the default shipping tax code from your [Tax Settings](https://dashboard.paystack.com/settings/tax) is used.
        """
    ...
    amount_total: int
    """
    Total amount after taxes in the [smallest currency unit](https://paystack.com/docs/currencies#zero-decimal).
    """
    currency: str
    """
    Three-letter [ISO currency code](https://www.iso.org/iso-4217-currency-codes.html), in lowercase. Must be a [supported currency](https://paystack.com/docs/currencies).
    ...
    """
    The ID of an existing [Customer](https://paystack.com/docs/api/customers/object) used for the resource.
    """
