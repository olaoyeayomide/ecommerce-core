...
from paystack._api_requestor import (
...
import warnings
from paystack._request_options import RequestOptions, extract_options_from_dict
from paystack._any_iterator import AnyIterator

T = TypeVar("T", bound=paystackObject)


class SearchResultObject(paystackObject, Generic[T]):
    OBJECT_NAME = "search_result"
    data: List[paystackObject]
    has_more: bool
    next_page: str
    ...
    @_util.deprecated(
        "This will be removed in a future version of paystack-python. Please call the `search` method on the corresponding resource directly, instead of the generic search on SearchResultObject."
    )
    ...
    #  Pyright doesn't like this because SearchResultObject inherits from paystackObject inherits from Dict[str, Any]
    ...
    def _maybe_empty_result(self, params: RequestOptions) -> Optional[Self]:
        if not self.has_more:
            ...
            return self._empty_search_result(
                ...
                paystack_version=options.get("paystack_version"),
                paystack_account=options.get("paystack_account"),
            )
        return None
