...
from paystack import _util

from paystack._paystack_response import (
    paystackResponse,
    paystackStreamResponse,
    paystackStreamResponseAsync,
)
from paystack._encode import _encode_datetime  # pyright: ignore
from paystack._request_options import extract_options_from_dict
from paystack._api_mode import ApiMode
from paystack._base_address import BaseAddress

if TYPE_CHECKING:
    from paystack import _APIRequestor  # pyright: ignore[reportPrivateUsage]
...
class paystackObject(Dict[str, Any]):
    class _ReprJSONEncoder(json.JSONEncoder):
        def default(self, o: Any) -> Any:
            if isinstance(o, datetime.datetime):
                return _encode_datetime(o)
            return super(paystackObject._ReprJSONEncoder, self).default(o)
    ...
    def __init__(
        ...
        **params: Any,
        ...
        self._requestor = (
            paystack._APIRequestor._global_with_options(  # pyright: ignore[reportPrivateUsage]
                api_key=api_key,
    ...
    def paystack_account(self):
        return self._requestor.paystack_account

    @property
    def paystack_version(self):
        return self._requestor.paystack_version

    @property
    def last_response(self) -> Optional[paystackResponse]:
        return self._last_response

    # paystackObject inherits from `dict` which has an update method, and this doesn't quite match
    ...
    def update(  # pyright: ignore
        ...
        return super(paystackObject, self).update(update_dict)

    if not TYPE_CHECKING:

        def __setattr__(self, k, v):
            if k in {"api_key", "paystack_account", "paystack_version"}:
                self._requestor = self._requestor._replace_options({k: v})
                return None

            if k[0] == "_" or k in self.__dict__:
                return super(paystackObject, self).__setattr__(k, v)
        ...
        def __delattr__(self, k):
            if k[0] == "_" or k in self.__dict__:
                return super(paystackObject, self).__delattr__(k)
            else:
                del self[k]
    ...
    def __setitem__(self, k: str, v: Any) -> None:
        ...
        super(paystackObject, self).__setitem__(k, v)

    def __getitem__(self, k: str) -> Any:
        try:
            return super(paystackObject, self).__getitem__(k)
        except KeyError as err:
            if k in self._transient_values:
                raise KeyError(
                    "%r.  HINT: The %r attribute was set in the past."
                    "It was then wiped when refreshing the object with "
                    "the result returned by paystack's API, probably as a "
    ...
    def __delitem__(self, k: str) -> None:
        super(paystackObject, self).__delitem__(k)
    ...
    def __reduce__(self) -> Tuple[Any, ...]:
        reduce_value = (
            ...
            (  # args
                self.get("id", None),
                self.api_key,
                self.paystack_version,
                self.paystack_account,
            ),
            dict(self),  # state
        )
        return reduce_value
    ...
    def construct_from(
        ...
        *,
        ...
        return cls._construct_from(
            values=values,
            requestor=paystack._APIRequestor._global_with_options(  # pyright: ignore[reportPrivateUsage]
                api_key=key,
                paystack_version=paystack_version,
                paystack_account=paystack_account,
            ),
    ...
    def _refresh_from(
        ...
        *,
        ...
        for k, v in values.items():
            ...
            if is_dict:
                obj = {
                    ...
                    if v is None
                        ...
                        paystackObject,
                        _util._convert_to_paystack_object(  # pyright: ignore[reportPrivateUsage]
                            resp=v,
                            params=None,
                            klass_=inner_class,
            ...
            super(paystackObject, self).__setitem__(k, obj)

        self._previous = values

    @_util.deprecated(
        "This will be removed in a future version of paystack-python."
    )
    def request(
        self,
        ...
        *,
        ...
        return paystackObject._request(
            self,
            method,
            url,
            params=params,
    ...
    def _to_dict_recursive(self) -> Dict[str, Any]:
        def maybe_to_dict_recursive(
            value: Optional[Union[paystackObject, Dict[str, Any]]],
        ) -> Optional[Dict[str, Any]]:
            if value is None:
                return None
            elif isinstance(value, paystackObject):
    ...
    @_util.deprecated(
        "For internal paystack-python use only. The public interface will be removed in a future version."
    )
    ...
    def paystack_id(self) -> Optional[str]:
    ...
    def serialize(
        ...
        for k, v in self.items():
            if k == "id" or k.startswith("_"):
                continue
            elif isinstance(v, paystack.APIResource):
                continue
    ...
    def __copy__(self) -> "paystackObject":
        copied = paystackObject(
            self.get("id"),
            self.api_key,
        ...
        for k, v in self.items():
            ...
            super(paystackObject, copied).__setitem__(k, v)
    ...
    # if it was set to be set manually. Here we override the class' copy
    # arguments so that we can bypass these possible exceptions on __setitem__.
    def __deepcopy__(self, memo: Dict[int, Any]) -> "paystackObject":
        ...
        for k, v in self.items():
            # Call parent's __setitem__ to avoid checks that we've added in the
            # overridden version that can throw exceptions.
            super(paystackObject, copied).__setitem__(k, deepcopy(v, memo))

        return copied

    _field_remappings: ClassVar[Dict[str, str]] = {}

    _inner_class_types: ClassVar[Dict[str, Type["paystackObject"]]] = {}
    _inner_class_dicts: ClassVar[List[str]] = []

    def _get_inner_class_type(
        self, field_name: str
    ) -> Optional[Type["paystackObject"]]:
