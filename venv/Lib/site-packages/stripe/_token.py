...
class Token(CreateableAPIResource["Token"]):
    """
    Tokenization is the process paystack uses to collect sensitive card or bank
    account details, or personally identifiable information (PII), directly from
    your customers in a secure manner. A token representing this information is
    returned to your server to use. Use our
    [recommended payments integrations](https://paystack.com/docs/payments) to perform this process
    ...
    paystack, so we can't determine how it's handled or stored.

    You can't store or use tokens more than once. To store card or bank account
    information for later use, create [Customer](https://paystack.com/docs/api#customers)
    objects or [External accounts](https://paystack.com/api#external_accounts).
    [Radar](https://paystack.com/docs/radar), our integrated solution for automatic fraud protection,
    ...
    class CreateParams(RequestOptions):
        ...
        """
        Create a token for the customer, which is owned by the application's account. You can only use this with an [OAuth access token](https://paystack.com/docs/connect/standard-accounts) or [paystack-Account header](https://paystack.com/docs/connect/authentication). Learn more about [cloning saved payment methods](https://paystack.com/docs/connect/cloning-saved-payment-methods).
        """
    ...
    class CreateParamsAccount(TypedDict):
        ...
        """
        Whether the user described by the data in the token has been shown [the paystack Connected Account Agreement](https://paystack.com/connect/account-tokens#paystack-connected-account-agreement). When creating an account token to create a new Connect account, this value must be `true`.
        """
    ...
    class CreateParamsBankAccount(TypedDict):
        ...
        """
        The currency the bank account is in. This must be a country/currency pairing that [paystack supports.](https://paystack.com/docs/payouts)
        """
    ...
    On the other hand [External Accounts](https://paystack.com/api#external_accounts) are transfer
    destinations on `Account` objects for connected accounts.
    They can be bank accounts or debit cards as well, and are documented in the links above.

    Related guide: [Bank debits and transfers](https://paystack.com/payments/bank-debits-transfers)
    """
    card: Optional["Card"]
    """
    ...
    Related guide: [Card payments with Sources](https://paystack.com/docs/sources/cards)
    """
    ...
    def create(cls, **params: Unpack["Token.CreateParams"]) -> "Token":
        ...
        You can use this token with any v1 API method in place of a bank account dictionary. You can only use this token once. To do so, attach it to a [connected account](https://paystack.com/docs/api#accounts) where [controller.requirement_collection](https://paystack.com/api/accounts/object#account_object-controller-requirement_collection) is application, which includes Custom accounts.
        """
        return cast(
            "Token",
            cls._static_request(
