...
if TYPE_CHECKING:
    ...
    from paystack._discount import Discount as DiscountResource
    from paystack._invoice import Invoice
    from paystack._line_item import LineItem
    from paystack._payment_intent import PaymentIntent
    from paystack._payment_link import PaymentLink
    ...
    from paystack._tax_id import TaxId as TaxIdResource
...
class Session(
    ...
    one-time purchases or subscriptions through [Checkout](https://paystack.com/docs/payments/checkout)
    or [Payment Links](https://paystack.com/docs/payments/payment-links). We recommend creating a
    new Session each time your customer attempts to pay.

    Once payment is successful, the Checkout Session will contain a reference
    to the [Customer](https://paystack.com/docs/api/customers), and either the successful
    [PaymentIntent](https://paystack.com/docs/api/payment_intents) or an active
    [Subscription](https://paystack.com/docs/api/subscriptions).

    You can create a Checkout Session on your server and redirect to its URL
    to begin Checkout.

    Related guide: [Checkout quickstart](https://paystack.com/docs/checkout/quickstart)
    """

    OBJECT_NAME: ClassVar[Literal["checkout.session"]] = "checkout.session"

    class AfterExpiration(paystackObject):
        class Recovery(paystackObject):
    ...
    class ConsentCollection(paystackObject):
        class PaymentMethodReuseAgreement(paystackObject):
            position: Literal["auto", "hidden"]
            """
            Determines the position and visibility of the payment method reuse agreement in the UI. When set to `auto`, paystack's defaults will be used.
    ...
    class CurrencyConversion(paystackObject):
        amount_subtotal: int
        """
    ...
    class CustomField(paystackObject):
        class Dropdown(paystackObject):
            class Option(paystackObject):
                label: str
                """
    ...
    class CustomText(paystackObject):
        class AfterSubmit(paystackObject):
            message: str
            """
            Text may be up to 1200 characters in length.
            """

        class ShippingAddress(paystackObject):
            message: str
            """
            Text may be up to 1200 characters in length.
            """

        class Submit(paystackObject):
            message: str
            """
            Text may be up to 1200 characters in length.
            """

        class TermsOfServiceAcceptance(paystackObject):
            message: str
            """
            Text may be up to 1200 characters in length.
            """
    ...
    class CustomerDetails(paystackObject):
    ...
    class InvoiceCreation(paystackObject):
        class InvoiceData(paystackObject):
            class CustomField(paystackObject):
                name: str
                """
            ...
            class RenderingOptions(paystackObject):
    ...
    class PaymentMethodOptions(paystackObject):
        class AcssDebit(paystackObject):
            class MandateOptions(paystackObject):
                ...
                """
                List of paystack products where this mandate can be selected automatically. Returned when the Session is in `setup` mode.
                """
    ...
    class PhoneNumberCollection(paystackObject):
        enabled: bool
        """
        Indicates whether phone number collection is enabled for the session
        """

    class SavedPaymentMethodOptions(paystackObject):
    ...
    class ShippingAddressCollection(paystackObject):
    ...
    class ShippingOption(paystackObject):
        shipping_amount: int
        """
    ...
    class TaxIdCollection(paystackObject):
        enabled: bool
        """
    ...
    class TotalDetails(paystackObject):
        class Breakdown(paystackObject):
            class Discount(paystackObject):
                amount: int
                """
                ...
                """
                A discount represents the actual application of a [coupon](https://paystack.com/docs/api#coupons) or [promotion code](https://paystack.com/docs/api#promotion_codes).
                It contains information about when the discount began, when it will end, and what it is applied to.

                Related guide: [Applying discounts to subscriptions](https://paystack.com/docs/billing/subscriptions/discounts)
                """
    ...
    class CreateParams(RequestOptions):
        ...
        """
        Three-letter [ISO currency code](https://www.iso.org/iso-4217-currency-codes.html), in lowercase. Must be a [supported currency](https://paystack.com/docs/currencies). Required in `setup` mode when `payment_method_types` is not set.
        """
        ...
        on the Checkout page. In `subscription` mode, the customer's [default payment method](https://paystack.com/docs/api/customers/update#update_customer-invoice_settings-default_payment_method)
        will be used if it's a card, otherwise the most recently saved card will be used. A valid billing address, billing name and billing email are required on the payment method for Checkout to prefill the customer's card details.

        If the Customer already has a valid [email](https://paystack.com/docs/api/customers/object#customer_object-email) set, the email will be prefilled and not editable in Checkout.
        If the Customer does not have a valid `email`, Checkout will set the email entered during the session on the Customer.

        If blank for Checkout Sessions in `subscription` mode or with `customer_creation` set as `always` in `payment` mode, Checkout will create a new Customer object based on information provided during the payment flow.

        You can set [`payment_intent_data.setup_future_usage`](https://paystack.com/docs/api/checkout/sessions/create#create_checkout_session-payment_intent_data-setup_future_usage) to have Checkout automatically attach the payment method to the Customer you pass in for future reuse.
        """
        customer_creation: NotRequired[Literal["always", "if_required"]]
        """
        Configure whether a Checkout Session creates a [Customer](https://paystack.com/docs/api/customers) during Session confirmation.

        When a Customer is not created, you can still retrieve email, address, and other customer data entered in Checkout
        with [customer_details](https://paystack.com/docs/api/checkout/sessions/object#checkout_session_object-customer_details).

        Sessions that don't create Customers instead are grouped by [guest customers](https://paystack.com/docs/payments/checkout/guest-customers)
        ...
        """
        A list of items the customer is purchasing. Use this parameter to pass one-time or recurring [Prices](https://paystack.com/docs/api/prices).
        ...
        If you'd like information on how to collect a payment method outside of Checkout, read the guide on configuring [subscriptions with a free trial](https://paystack.com/docs/payments/checkout/free-trials).
        """
        ...
        You can omit this attribute to manage your payment methods from the [paystack Dashboard](https://dashboard.paystack.com/settings/payment_methods).
        See [Dynamic Payment Methods](https://paystack.com/docs/payments/payment-methods/integration-options#using-dynamic-payment-methods) for more details.

        Read more about the supported payment methods and their requirements in our [payment
        method details guide](https://paystack.com/docs/payments/checkout/payment-methods).
        ...
        before using this feature. Learn more about [collecting phone numbers with Checkout](https://paystack.com/docs/payments/checkout/phone-numbers).
        """
        redirect_on_completion: NotRequired[
            Literal["always", "if_required", "never"]
        ]
        """
        This parameter applies to `ui_mode: embedded`. Learn more about the [redirect behavior](https://paystack.com/docs/payments/checkout/custom-success-page?payment-ui=embedded-form) of embedded sessions. Defaults to `always`.
        """
        ...
        """
        The URL to which paystack should send customers when payment or setup
        is complete.
        This parameter is not allowed if ui_mode is `embedded`. If you'd like to use
        information from the successful Checkout Session on your page, read the
        guide on [customizing your success page](https://paystack.com/docs/payments/checkout/custom-success-page).
        """
    ...
    class CreateParamsConsentCollection(TypedDict):
        ...
        There must be a valid terms of service URL set in your [Dashboard settings](https://dashboard.paystack.com/settings/public).
        """

    class CreateParamsConsentCollectionPaymentMethodReuseAgreement(TypedDict):
        position: Literal["auto", "hidden"]
        """
        Determines the position and visibility of the payment method reuse agreement in the UI. When set to `auto`, paystack's
    ...
    class CreateParamsLineItem(TypedDict):
        ...
        """
        The [tax rates](https://paystack.com/docs/api/tax_rates) that will be applied to this line item depending on the customer's billing/shipping address. We currently support the following countries: US, GB, AU, and all countries in the EU.
        """
        price: NotRequired[str]
        """
        The ID of the [Price](https://paystack.com/docs/api/prices) or [Plan](https://paystack.com/docs/api/plans) object. One of `price` or `price_data` is required.
        """
        ...
        """
        The [tax rates](https://paystack.com/docs/api/tax_rates) which apply to this line item.
        """
    ...
    class CreateParamsPaymentIntentData(TypedDict):
        ...
        """
        The paystack account ID for which these funds are intended. For details,
        see the PaymentIntents [use case for connected
        accounts](https://paystack.com/docs/payments/connected-accounts).
        """
        ...
        """
        Indicates that you intend to [make future payments](https://paystack.com/docs/payments/payment-intents#future-usage) with the payment
        ...
        For more information, see the PaymentIntents [use case for connected accounts](https://paystack.com/docs/payments/connected-accounts).
        """
        transfer_group: NotRequired[str]
        """
        A string that identifies the resulting payment as part of a group. See the PaymentIntents [use case for connected accounts](https://paystack.com/docs/connect/separate-charges-and-transfers) for details.
        """
    ...
    class CreateParamsPaymentMethodOptionsAcssDebit(TypedDict):
        ...
        """
        Three-letter [ISO currency code](https://www.iso.org/iso-4217-currency-codes.html), in lowercase. Must be a [supported currency](https://paystack.com/docs/currencies). This is only accepted for Checkout Sessions in `setup` mode.
        """
    ...
    class CreateParamsPaymentMethodOptionsAcssDebitMandateOptions(TypedDict):
        ...
        """
        List of paystack products where this mandate can be selected automatically. Only usable in `setup` mode.
        """
    ...
    class CreateParamsSetupIntentData(TypedDict):
        ...
        on_behalf_of: NotRequired[str]
        """
        The paystack account for which the setup is intended.
        """
    ...
    class CreateParamsSubscriptionData(TypedDict):
        ...
        """
        A non-negative decimal between 0 and 100, with at most two decimal places. This represents the percentage of the subscription invoice total that will be transferred to the application owner's paystack account. To use an application fee percent, the request must be made on behalf of another account, using the `paystack-Account` header or an OAuth key. For more information, see the application fees [documentation](https://paystack.com/docs/connect/subscriptions#collecting-fees-on-subscriptions).
        """
        ...
        for rendering in the [customer portal](https://paystack.com/docs/customer-management).
        """
    ...
    """
    Client secret to be used when initializing paystack.js embedded checkout.
    """
    ...
    """
    Currency conversion details for [Adaptive Pricing](https://docs.paystack.com/payments/checkout/adaptive-pricing) sessions
    """
    ...
    """
    The ID of the PaymentIntent for Checkout Sessions in `payment` mode. You can't confirm or cancel the PaymentIntent for a Checkout Session. To cancel, [expire the Checkout Session](https://paystack.com/docs/api/checkout/sessions/expire) instead.
    """
    ...
    """
    The ID of the SetupIntent for Checkout Sessions in `setup` mode. You can't confirm or cancel the SetupIntent for a Checkout Session. To cancel, [expire the Checkout Session](https://paystack.com/docs/api/checkout/sessions/expire) instead.
    """
    ...
    url: Optional[str]
    """
    The URL to the Checkout Session. Redirect customers to this URL to take them to Checkout. If you're using [Custom Domains](https://paystack.com/docs/payments/checkout/custom-domains), the URL will use your subdomain. Otherwise, it'll use `checkout.paystack.com.`
