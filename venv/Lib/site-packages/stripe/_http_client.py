...
from paystack._request_metrics import RequestMetrics
from paystack._error import APIConnectionError
...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                "Warning: the paystack library requires that your Python "
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. paystack will fall back to '
                "an alternate HTTP library so everything should work. We "
                'recommend upgrading your "requests" library. If you have any '
                "questions, please contact support@paystack.com. (HINT: running "
...
class HTTPClient(object):
    ...
    def _should_retry(
        ...
        if rheaders is not None and "paystack-should-retry" in rheaders:
            if rheaders["paystack-should-retry"] == "false":
                return False
            if rheaders["paystack-should-retry"] == "true":
                return True
        ...
        #
        # Note that we expect the paystack-should-retry header to be false
    ...
    def _add_telemetry_header(
        ...
        if paystack.enable_telemetry and last_request_metrics:
            telemetry = {
                "last_request_metrics": last_request_metrics.payload()
            }
            ret = dict(headers)
            ret["X-paystack-Client-Telemetry"] = json.dumps(telemetry)
            return ret
        return headers

    def _record_request_metrics(self, response, request_start, usage):
        _, _, rheaders = response
        if "Request-Id" in rheaders and paystack.enable_telemetry:
...
class RequestsClient(HTTPClient):
    ...
    def _request_internal(
        ...
        if self._verify_ssl_certs:
            kwargs["verify"] = paystack.ca_bundle_path
        else:
            kwargs["verify"] = False
    ...
    def _handle_request_error(self, e: Exception) -> NoReturn:
        ...
        if isinstance(e, self.requests.exceptions.SSLError):
            msg = (
                "Could not verify paystack's SSL certificate.  Please make "
        ...
        # Retry only timeout and connect errors; similar to urllib3 Retry
            ...
            msg = (
                "Unexpected error communicating with paystack.  "
                "If this problem persists, let us know at "
                "support@paystack.com."
            )
            err = "%s: %s" % (type(e).__name__, str(e))
...
class UrlFetchClient(HTTPClient):
    ...
    def _handle_request_error(self, e: Exception, url: str) -> NoReturn:
        if isinstance(e, self.urlfetch.InvalidURLError):
            msg = (
                "The paystack library attempted to fetch an "
                "invalid URL (%r). This is likely due to a bug "
                "in the paystack Python bindings. Please let us know "
                "at support@paystack.com." % (url,)
            )
        elif isinstance(e, self.urlfetch.DownloadError):
            msg = "There was a problem retrieving data from paystack."
        elif isinstance(e, self.urlfetch.ResponseTooLargeError):
            msg = (
                "There was a problem receiving all of your data from "
                "paystack.  This is likely due to a bug in paystack. "
                "Please let us know at support@paystack.com."
            )
        else:
            msg = (
                "Unexpected error communicating with paystack. If this "
                "problem persists, let us know at support@paystack.com."
            )
...
class PycurlClient(HTTPClient):
    ...
    def _request_internal(
        ...
        if self._verify_ssl_certs:
            self._curl.setopt(self.pycurl.CAINFO, paystack.ca_bundle_path)
        else:
    ...
    def _handle_request_error(self, e: Exception) -> NoReturn:
        if e.args[0] in [
            ...
            msg = (
                "Could not connect to paystack.  Please check your "
                "internet connection and try again.  If this problem "
                "persists, you should check paystack's service status at "
                "https://twitter.com/paystackstatus, or let us know at "
                "support@paystack.com."
            )
            should_retry = True
        elif e.args[0] in [
...
class Urllib2Client(HTTPClient):
    ...
    def _handle_request_error(self, e: Exception) -> NoReturn:
        msg = (
            "Unexpected error communicating with paystack. "
            "If this problem persists, let us know at support@paystack.com."
        )
        msg = textwrap.fill(msg) + "\n\n(Network error: " + str(e) + ")"
        raise APIConnectionError(msg) from e
...
class HTTPXClient(HTTPClient):
    ...
    def request(
        ...
        if self._client is None:
            raise RuntimeError(
                "paystack: HTTPXClient was initialized with allow_sync_methods=False, "
    ...
    def request_stream(
        ...
        if self._client is None:
            raise RuntimeError(
                "paystack: HTTPXClient was not initialized with allow_sync_methods=True, "
                "so it cannot be used for synchronous requests."
            )
        args, kwargs = self._get_request_args_kwargs(
            method, url, headers, post_data
        ...
        if self._cached_session is None:
            ...
            if self._verify_ssl_certs:
                ssl_context = ssl.create_default_context(
                    cafile=paystack.ca_bundle_path
                )
    ...
    def raise_async_client_import_error() -> Never:
        raise ImportError(
            (
                ...
                "async http client by subclassing paystack.HTTPClient"
                "and setting paystack.default_http_client to an instance of it."
            )
        )
