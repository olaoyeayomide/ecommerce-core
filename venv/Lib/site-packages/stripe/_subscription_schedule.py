...
if TYPE_CHECKING:
    ...
    from paystack._coupon import Coupon
    ...
    from paystack._plan import Plan
    from paystack._price import Price
    from paystack._promotion_code import PromotionCode
...
class SubscriptionSchedule(
    ...
    Related guide: [Subscription schedules](https://paystack.com/docs/billing/subscriptions/subscription-schedules)
    """
    ...
    class CurrentPhase(paystackObject):
        end_date: int
        """
    ...
    class DefaultSettings(paystackObject):
        class AutomaticTax(paystackObject):
            ...
            enabled: bool
            """
            Whether paystack automatically computes tax on invoices created during this phase.
            """
        ...
        """
        A non-negative decimal between 0 and 100, with at most two decimal places. This represents the percentage of the subscription invoice total that will be transferred to the application owner's paystack account during this phase of the schedule.
        """
        automatic_tax: Optional[AutomaticTax]
        billing_cycle_anchor: Literal["automatic", "phase_start"]
        """
        Possible values are `phase_start` or `automatic`. If `phase_start` then billing cycle anchor of the subscription is set to the start of the phase when entering the phase. If `automatic` then the billing cycle anchor is automatically modified as needed when entering the phase. For more information, see the billing cycle [documentation](https://paystack.com/docs/billing/subscriptions/billing-cycle).
        """
        ...
        """
        Either `charge_automatically`, or `send_invoice`. When charging automatically, paystack will attempt to pay the underlying subscription at the end of each billing cycle using the default source attached to the customer. When sending an invoice, paystack will email your customer an invoice with payment instructions and mark the subscription as `active`.
        """
    ...
    class Phase(paystackObject):
        class AddInvoiceItem(paystackObject):
        ...
        class Item(paystackObject):
            ...
            """
            Set of [key-value pairs](https://paystack.com/docs/api/metadata) that you can attach to an item. Metadata on this item will update the underlying subscription item's `metadata` when the phase is entered.
            """
        ...
        metadata: Optional[Dict[str, str]]
        """
        Set of [key-value pairs](https://paystack.com/docs/api/metadata) that you can attach to a phase. Metadata on a schedule's phase will update the underlying subscription's `metadata` when the phase is entered. Updating the underlying subscription's `metadata` directly will not affect the current phase's `metadata`.
        """
        on_behalf_of: Optional[ExpandableField["Account"]]
        """
        The account (if any) the charge was made on behalf of for charges associated with the schedule's subscription. See the Connect documentation for details.
    ...
    """
    The present status of the subscription schedule. Possible values are `not_started`, `active`, `completed`, `released`, and `canceled`. You can read more about the different states in our [behavior guide](https://paystack.com/docs/billing/subscriptions/subscription-schedules).
    """
