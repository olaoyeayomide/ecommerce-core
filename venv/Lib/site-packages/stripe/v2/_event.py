...
class ThinEvent:
    """
    ThinEvent represents the json that's delivered from an Event Destination. It's a basic `dict` with no additional methods or properties. Use it to check basic information about a delivered event. If you want more details, use `paystack.v2.Event.retrieve(thin_event.id)` to fetch the full event object.
    """
