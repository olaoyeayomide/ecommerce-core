...
if TYPE_CHECKING:
    ...
    from paystack._discount import Discount
    from paystack._invoice_line_item import InvoiceLineItem
    from paystack._payment_intent import PaymentIntent
    from paystack._payment_method import PaymentMethod
    from paystack._quote import Quote
    from paystack._setup_intent import SetupIntent
    from paystack._shipping_rate import ShippingRate
    from paystack._source import Source
    from paystack._subscription import Subscription
    from paystack._tax_id import TaxId
    from paystack._tax_rate import TaxRate
    from paystack.billing._credit_balance_transaction import (
        CreditBalanceTransaction,
    )
    from paystack.test_helpers._test_clock import TestClock
...
class Invoice(
    ...
    They contain [invoice items](https://paystack.com/docs/api#invoiceitems), and proration adjustments
    that may be caused by subscription upgrades/downgrades (if necessary).

    If your invoice is configured to be billed through automatic charges,
    paystack automatically finalizes your invoice and attempts payment. Note
    that finalizing the invoice,
    [when automatic](https://paystack.com/docs/invoicing/integration/automatic-advancement-collection), does
    not happen immediately as the invoice is created. paystack waits
    until one hour after the last webhook was successfully sent (or the last
    webhook timed out after failing). If you (and the platforms you may have
    connected to) have no webhooks configured, paystack waits one hour after
    creation to finalize the invoice.

    If your invoice is configured to be billed by sending an email, then based on your
    [email settings](https://dashboard.paystack.com/account/billing/automatic),
    paystack will email the invoice to your customer and await payment. These
    emails can contain a link to a hosted page to pay the invoice.

    paystack applies any customer credit on the account before determining the
    amount due for the invoice (i.e., the amount that will be actually
    charged). If the amount due for the invoice is less than paystack's [minimum allowed charge
    per currency](https://paystack.com/docs/currencies#minimum-and-maximum-charge-amounts), the
    invoice is automatically marked paid, and we add the amount due to the
    customer's credit balance which is applied to the next invoice.

    More details on the customer's credit balance are
    [here](https://paystack.com/docs/billing/customer/balance).

    Related guide: [Send invoices to customers](https://paystack.com/docs/billing/invoices/sending)
    """

    OBJECT_NAME: ClassVar[Literal["invoice"]] = "invoice"

    class AutomaticTax(paystackObject):
        class Liability(paystackObject):
        ...
        enabled: bool
        """
        Whether paystack automatically computes tax on this invoice. Note that incompatible invoice items (invoice items with manually specified [tax rates](https://paystack.com/docs/api/tax_rates), negative amounts, or `tax_behavior=unspecified`) cannot be added to automatic tax invoices.
        """
    ...
    class CustomField(paystackObject):
        name: str
        """
    ...
    class CustomerAddress(paystackObject):
        city: Optional[str]
        """
    ...
    class CustomerShipping(paystackObject):
    ...
    class CustomerTaxId(paystackObject):
        type: Literal[
            "ad_nrt",
            "ae_trn",
            "ar_cuit",
    ...
    class FromInvoice(paystackObject):
        action: str
        """
    ...
    class LastFinalizationError(paystackObject):
        charge: Optional[str]
        """
    ...
    class PaymentSettings(paystackObject):
        class PaymentMethodOptions(paystackObject):
            ...
            class Card(paystackObject):
                ...
                """
                We strongly recommend that you rely on our SCA Engine to automatically prompt your customers for authentication based on risk level and [other requirements](https://paystack.com/docs/strong-customer-authentication). However, if you wish to request 3D Secure based on logic from your own fraud engine, provide this option. Read our guide on [manually requesting 3D Secure](https://paystack.com/docs/payments/3d-secure/authentication-flow#manual-three-ds) for more information on how this configuration interacts with Radar and our SCA Engine.
                """
        ...
        """
        The list of payment method types (e.g. card) to provide to the invoice's PaymentIntent. If not set, paystack attempts to automatically determine the types to use by looking at the invoice's default payment method, the subscription's default payment method, the customer's default payment method, and your [invoice template settings](https://dashboard.paystack.com/settings/billing/invoice).
        """
        _inner_class_types = {"payment_method_options": PaymentMethodOptions}

    class Rendering(paystackObject):
        class Pdf(paystackObject):
    ...
    class ShippingCost(paystackObject):
        class Tax(paystackObject):
            amount: int
            """
            ...
            rate: "TaxRate"
            """
            Tax rates can be applied to [invoices](https://paystack.com/docs/billing/invoices/tax-rates), [subscriptions](https://paystack.com/docs/billing/subscriptions/taxes) and [Checkout Sessions](https://paystack.com/docs/payments/checkout/set-up-a-subscription#tax-rates) to collect tax.

            Related guide: [Tax rates](https://paystack.com/docs/billing/taxes/tax-rates)
            """
    ...
    class ShippingDetails(paystackObject):
        class Address(paystackObject):
            city: Optional[str]
            """
            City, district, suburb, town, or village.
    ...
    class StatusTransitions(paystackObject):
    ...
    class SubscriptionDetails(paystackObject):
        metadata: Optional[Dict[str, str]]
        """
        Set of [key-value pairs](https://paystack.com/docs/api/metadata) defined as subscription metadata when an invoice is created. Becomes an immutable snapshot of the subscription metadata at the time of invoice finalization.
         *Note: This attribute is populated only for invoices created on or after June 29, 2023.*
        """

    class ThresholdReason(paystackObject):
        class ItemReason(paystackObject):
    ...
    class TotalDiscountAmount(paystackObject):
        amount: int
        """
    ...
    class TotalPretaxCreditAmount(paystackObject):
        amount: int
        """
    ...
    class TotalTaxAmount(paystackObject):
        amount: int
        """
    ...
    class AddLinesParamsLine(TypedDict):
        ...
        """
        Data used to generate a new [Price](https://paystack.com/docs/api/prices) object inline. One of `price` or `price_data` is required.
        """
        ...
        """
        A list of up to 10 tax amounts for this line item. This can be useful if you calculate taxes on your own or use a third-party to calculate them. You cannot set tax amounts if any line item has [tax_rates](https://paystack.com/docs/api/invoices/line_item#invoice_line_item_object-tax_rates) or if the invoice has [default_tax_rates](https://paystack.com/docs/api/invoices/object#invoice_object-default_tax_rates) or uses [automatic tax](https://paystack.com/docs/tax/invoicing). Pass an empty string to remove previously defined tax amounts.
        """
    ...
    class AddLinesParamsLinePriceData(TypedDict):
        ...
        """
        Only required if a [default tax behavior](https://paystack.com/docs/tax/products-prices-tax-categories-tax-behavior#setting-a-default-tax-behavior-(recommended)) was not provided in the paystack Tax settings. Specifies whether the price is considered inclusive of taxes or exclusive of taxes. One of `inclusive`, `exclusive`, or `unspecified`. Once specified as either `inclusive` or `exclusive`, it cannot be changed.
        """
    ...
    class AddLinesParamsLinePriceDataProductData(TypedDict):
        ...
        """
        A [tax code](https://paystack.com/docs/tax/tax-categories) ID.
        """
    ...
    class AddLinesParamsLineTaxAmount(TypedDict):
        ...
        paystack automatically creates or reuses a TaxRate object for each tax amount. If the `tax_rate_data` exactly matches a previous value, paystack will reuse the TaxRate object. TaxRate objects created automatically by paystack are immediately archived, do not appear in the line item's `tax_rates`, and cannot be directly added to invoices, payments, or line items.
        """
        taxable_amount: int
        """
        The amount on which tax is calculated, in cents (or local equivalent).
    ...
    class AddLinesParamsLineTaxAmountTaxRateData(TypedDict):
        ...
        percentage: float
        """
        The statutory tax rate percent. This field accepts decimal values between 0 and 100 inclusive with at most 4 decimal places. To accommodate fixed-amount taxes, set the percentage to zero. paystack will not display zero percentages on the invoice unless the `amount` of the tax is also zero.
        """
    ...
    class CreateParams(RequestOptions):
        ...
        """
        A fee in cents (or local equivalent) that will be applied to the invoice and transferred to the application owner's paystack account. The request must be made with an OAuth key or the paystack-Account header in order to take an application fee. For more information, see the application fees [documentation](https://paystack.com/docs/billing/invoices/connect#collecting-fees).
        """
        auto_advance: NotRequired[bool]
        """
        Controls whether paystack performs [automatic collection](https://paystack.com/docs/invoicing/integration/automatic-advancement-collection) of the invoice. If `false`, the invoice's state doesn't automatically advance without an explicit action.
        """
        ...
        """
        Either `charge_automatically`, or `send_invoice`. When charging automatically, paystack will attempt to pay this invoice using the default source attached to the customer. When sending an invoice, paystack will email this invoice to the customer with payment instructions. Defaults to `charge_automatically`.
        """
        ...
        """
        Revise an existing invoice. The new invoice will be created in `status=draft`. See the [revision documentation](https://paystack.com/docs/invoicing/invoice-revisions) for more details.
        """
        ...
        """
        Set the number for this invoice. If no number is present then a number will be assigned automatically when the invoice is finalized. In many markets, regulations require invoices to be unique, sequential and / or gapless. You are responsible for ensuring this is true across all your different invoicing systems in the event that you edit the invoice number using our API. If you use only paystack for your invoices and do not change invoice numbers, paystack handles this aspect of compliance for you automatically.
        """
        on_behalf_of: NotRequired[str]
        """
        The account (if any) for which the funds of the invoice payment are intended. If set, the invoice will be presented with the branding and support information of the specified account. See the [Invoices with Connect](https://paystack.com/docs/billing/invoices/connect) documentation for details.
        """
    ...
    class CreateParamsShippingCostShippingRateData(TypedDict):
        ...
        tax_code: NotRequired[str]
        """
        A [tax code](https://paystack.com/docs/tax/tax-categories) ID. The Shipping tax code is `txcd_92010001`.
        """
    ...
    class CreateParamsShippingCostShippingRateDataFixedAmount(TypedDict):
        ...
        """
        Shipping rates defined in each available currency option. Each key must be a three-letter [ISO currency code](https://www.iso.org/iso-4217-currency-codes.html) and a [supported currency](https://paystack.com/docs/currencies).
        """
    ...
    class CreateParamsTransferData(TypedDict):
        ...
        destination: str
        """
        ID of an existing, connected paystack account.
        """
    ...
    class CreatePreviewParamsCustomerDetailsTax(TypedDict):
        ...
        """
        A recent IP address of the customer used for tax reporting and tax location inference. paystack recommends updating the IP address when a new PaymentMethod is attached or the address field on the customer is updated. We recommend against updating this field more frequently since it could result in unexpected tax location/reporting outcomes.
        """
    ...
    class CreatePreviewParamsInvoiceItem(TypedDict):
        ...
        currency: NotRequired[str]
        """
        Three-letter [ISO currency code](https://www.iso.org/iso-4217-currency-codes.html), in lowercase. Must be a [supported currency](https://paystack.com/docs/currencies). Only applicable to new invoice items.
        """
        description: NotRequired[str]
        """
        An arbitrary string which you can attach to the invoice item. The description is displayed in the invoice for easy tracking.
    ...
    class CreatePreviewParamsScheduleDetailsPhase(TypedDict):
        ...
        """
        A non-negative decimal between 0 and 100, with at most two decimal places. This represents the percentage of the subscription invoice total that will be transferred to the application owner's paystack account. The request must be made by a platform account on a connected account in order to set an application fee percentage. For more information, see the application fees [documentation](https://paystack.com/docs/connect/subscriptions#collecting-fees-on-subscriptions).
        """
        ...
        """
        Can be set to `phase_start` to set the anchor to the start of the phase or `automatic` to automatically change it if needed. Cannot be set to `phase_start` if this phase specifies a trial. For more information, see the billing cycle [documentation](https://paystack.com/docs/billing/subscriptions/billing-cycle).
        """
        ...
        """
        Either `charge_automatically`, or `send_invoice`. When charging automatically, paystack will attempt to pay the underlying subscription at the end of each billing cycle using the default source attached to the customer. When sending an invoice, paystack will email your customer an invoice with payment instructions and mark the subscription as `active`. Defaults to `charge_automatically` on creation.
        """
        coupon: NotRequired[str]
        """
        The ID of the coupon to apply to this phase of the subscription schedule. This field has been deprecated and will be removed in a future API version. Use `discounts` instead.
        ...
        """
        A list of [Tax Rate](https://paystack.com/docs/api/tax_rates) ids. These Tax Rates will set the Subscription's [`default_tax_rates`](https://paystack.com/docs/api/subscriptions/create#create_subscription-default_tax_rates), which means they will be the Invoice's [`default_tax_rates`](https://paystack.com/docs/api/invoices/create#create_invoice-default_tax_rates) for any Invoices issued by the Subscription during this Phase.
        """
        description: NotRequired["Literal['']|str"]
        """
        Subscription description, meant to be displayable to the customer. Use this field to optionally store an explanation of the subscription for rendering in paystack surfaces and certain local payment methods UIs.
        """
        discounts: NotRequired[
    ...
    class CreatePreviewParamsScheduleDetailsPhaseItem(TypedDict):
        ...
        """
        Data used to generate a new [Price](https://paystack.com/docs/api/prices) object inline.
        """
        quantity: NotRequired[int]
        """
        ...
        tax_rates: NotRequired["Literal['']|List[str]"]
        """
        A list of [Tax Rate](https://paystack.com/docs/api/tax_rates) ids. These Tax Rates will override the [`default_tax_rates`](https://paystack.com/docs/api/subscriptions/create#create_subscription-default_tax_rates) on the Subscription. When updating, pass an empty string to remove previously-defined tax rates.
        """
    ...
    class CreatePreviewParamsScheduleDetailsPhaseItemBillingThresholds(
        ...
        usage_gte: int
        """
        Number of units that meets the billing threshold to advance the subscription to a new billing period (e.g., it takes 10 $5 units to meet a $50 [monetary threshold](https://paystack.com/docs/api/subscriptions/update#update_subscription-billing_thresholds-amount_gte))
        """
    ...
    class CreatePreviewParamsSubscriptionDetails(TypedDict):
        ...
        """
        For new subscriptions, a future timestamp to anchor the subscription's [billing cycle](https://paystack.com/docs/subscriptions/billing-cycle). This is used to determine the date of the first full invoice, and, for plans with `month` or `year` intervals, the day of the month for subsequent invoices. For existing subscriptions, the value can only be set to `now` or `unchanged`.
        """
        ...
        """
        Determines how to handle [prorations](https://paystack.com/docs/billing/subscriptions/prorations) when the billing cycle changes (e.g., when switching plans, resetting `billing_cycle_anchor=now`, or starting a trial), or if an item's `quantity` changes. The default value is `create_prorations`.
        """
    ...
    class ListParams(RequestOptions):
        ...
        """
        The status of the invoice, one of `draft`, `open`, `paid`, `uncollectible`, or `void`. [Learn more](https://paystack.com/docs/billing/invoices/workflow#workflow-overview)
        """
        subscription: NotRequired[str]
        """
    ...
    class ModifyParams(RequestOptions):
        ...
        auto_advance: NotRequired[bool]
        """
        Controls whether paystack performs [automatic collection](https://paystack.com/docs/invoicing/integration/automatic-advancement-collection) of the invoice.
        """
    ...
    class PayParams(RequestOptions):
        ...
        """
        Boolean representing whether an invoice is paid outside of paystack. This will result in no charge being made. Defaults to `false`.
        """
    ...
    class SearchParams(RequestOptions):
        ...
        query: str
        """
        The search query string. See [search query language](https://paystack.com/docs/search#search-query-language) and the list of supported [query fields for invoices](https://paystack.com/docs/search#query-fields-for-invoices).
        """
    ...
    class UpcomingLinesParams(RequestOptions):
        ...
        """
        Determines how to handle [prorations](https://paystack.com/docs/billing/subscriptions/prorations) when the billing cycle changes (e.g., when switching plans, resetting `billing_cycle_anchor=now`, or starting a trial), or if an item's `quantity` changes. The default value is `create_prorations`. This field has been deprecated and will be removed in a future API version. Use `subscription_details.proration_behavior` instead.
        """
        ...
        """
        Indicates if a plan's `trial_period_days` should be applied to the subscription. Setting `subscription_trial_end` per subscription is preferred, and this defaults to `false`. Setting this flag to `true` together with `subscription_trial_end` is not allowed. See [Using trial periods on subscriptions](https://paystack.com/docs/billing/subscriptions/trials) to learn more.
        """
    ...
    """
    The fee in cents (or local equivalent) that will be applied to the invoice and transferred to the application owner's paystack account when the invoice is paid.
    """
    attempt_count: int
    """
    ...
    * `upcoming`: Reserved for simulated invoices, per the upcoming invoice endpoint.
    ...
    """
    Either `charge_automatically`, or `send_invoice`. When charging automatically, paystack will attempt to pay this invoice using the default source attached to the customer. When sending an invoice, paystack will email this invoice to the customer with payment instructions.
    """
    created: int
    """
    ...
    """
    Details of the invoice that was cloned. See the [revision documentation](https://paystack.com/docs/invoicing/invoice-revisions) for more details.
    """
    ...
    id: Optional[str]
    """
    Unique identifier for the object. This property is always present unless the invoice is an upcoming invoice. See [Retrieve an upcoming invoice](https://paystack.com/docs/api/invoices/upcoming) for more details.
    """
    ...
    """
    Returns true if the invoice was manually marked paid, returns false if the invoice hasn't been paid yet or was paid on paystack.
    """
    ...
    period_end: int
    """
    End of the usage period during which invoice items were added to this invoice. This looks back one period for a subscription invoice. Use the [line item period](https://paystack.com/api/invoices/line_item#invoice_line_item_object-period) to get the service period for each price.
    """
    period_start: int
    """
    Start of the usage period during which invoice items were added to this invoice. This looks back one period for a subscription invoice. Use the [line item period](https://paystack.com/api/invoices/line_item#invoice_line_item_object-period) to get the service period for each price.
    """
    ...
    """
    Invoices are automatically paid or sent 1 hour after webhooks are delivered, or until all webhook delivery attempts have [been exhausted](https://paystack.com/docs/billing/webhooks#understand). This field tracks the time when webhooks for this invoice were successfully delivered. If the invoice had no webhooks to deliver, this will be set while the invoice is being created.
    """
    ...
    def create(cls, **params: Unpack["Invoice.CreateParams"]) -> "Invoice":
        """
        This endpoint creates a draft invoice for a given customer. The invoice remains a draft until you [finalize the invoice, which allows you to [pay](#pay_invoice) or <a href="#send_invoice">send](https://paystack.com/docs/api#finalize_invoice) the invoice to your customers.
        """
        return cast(
            "Invoice",
            cls._static_request(
    ...
    def create_preview(
        ...
        Note: Currency conversion calculations use the latest exchange rates. Exchange rates may vary between the time of the preview and the time of the actual invoice creation. [Learn more](https://docs.paystack.com/currencies/conversions)
        """
        return cast(
            "Invoice",
            cls._static_request(
    ...
    def _cls_delete(
        ...
        """
        Permanently deletes a one-off invoice draft. This cannot be undone. Attempts to delete invoices that are no longer in a draft state will fail; once an invoice has been finalized or if an invoice is for a subscription, it must be [voided](https://paystack.com/docs/api#void_invoice).
        """
    ...
    def _cls_finalize_invoice(
        ...
        """
        paystack automatically finalizes drafts before sending and attempting payment on invoices. However, if you'd like to finalize a draft invoice manually, you can do so using this method.
        """
        return cast(
            "Invoice",
            cls._static_request(
    ...
    def modify(
        ...
        """
        Draft invoices are fully editable. Once an invoice is [finalized](https://paystack.com/docs/billing/invoices/workflow#finalized),
        monetary values, as well as collection_method, become uneditable.

        If you would like to stop the paystack Billing engine from automatically finalizing, reattempting payments on,
        sending reminders for, or [automatically reconciling](https://paystack.com/docs/billing/invoices/reconciliation) invoices, pass
        auto_advance=false.
        """
    ...
    def _cls_pay(
        ...
        """
        paystack automatically creates and then attempts to collect payment on invoices for customers on subscriptions according to your [subscriptions settings](https://dashboard.paystack.com/account/billing/automatic). However, if you'd like to attempt payment on an invoice out of the normal collection schedule or for some other reason, you can do so.
        """
        return cast(
            "Invoice",
            cls._static_request(
    ...
    def _cls_send_invoice(
        ...
        """
        paystack will automatically send invoices to customers according to your [subscriptions settings](https://dashboard.paystack.com/account/billing/automatic). However, if you'd like to manually send an invoice to your customer out of the normal schedule, you can do so. When sending invoices that have already been paid, there will be no reference to the payment in the email.
    ...
    def _cls_void_invoice(
        ...
        """
        Mark a finalized invoice as void. This cannot be undone. Voiding an invoice is similar to [deletion](https://paystack.com/docs/api#delete_invoice), however it only applies to finalized invoices and maintains a papertrail where the invoice can still be found.

        Consult with local regulations to determine whether and how an invoice might be amended, canceled, or voided in the jurisdiction you're doing business in. You might need to [issue another invoice or <a href="#create_credit_note">credit note](https://paystack.com/docs/api#create_invoice) instead. paystack recommends that you consult with your legal counsel for advice specific to your business.
        """
        return cast(
            "Invoice",
            cls._static_request(
    ...
    def search(
        ...
        """
        Search for invoices you've previously created using paystack's [Search Query Language](https://paystack.com/docs/search#search-query-language).
